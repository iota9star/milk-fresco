...
public class SkyViewHolder extends BaseViewHolder<SkyBean> {
    ...
    public SkyViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<SkyBean> beans) {
        final SkyBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<SkyBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (SkyBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_ACG_GAMERSKY,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
