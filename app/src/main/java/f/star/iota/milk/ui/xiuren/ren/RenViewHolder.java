...
public class RenViewHolder extends BaseViewHolder<RenBean> {
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<RenBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (RenBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_XIUREN,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
