...
public class StationViewHolder extends BaseViewHolder<StationBean> {
    ...
    @Override
    public void bindView(final List<StationBean> beans) {
        final StationBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<StationBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (StationBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_ARTSTATION,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
