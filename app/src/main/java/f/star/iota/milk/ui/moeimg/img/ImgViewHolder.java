...
public class ImgViewHolder extends BaseViewHolder<ImgBean> {
    ...
    @Override
    public void bindView(final List<ImgBean> beans) {
        final ImgBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<ImgBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (ImgBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_MOEIMG,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
