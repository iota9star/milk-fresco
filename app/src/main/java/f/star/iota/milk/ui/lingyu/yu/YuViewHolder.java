package f.star.iota.milk.ui.lingyu.yu;
...
public class YuViewHolder extends BaseViewHolder<YuBean> {
    ...
    @Override
    public void bindView(final List<YuBean> beans) {
        final YuBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<YuBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (YuBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_LINGYU,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
