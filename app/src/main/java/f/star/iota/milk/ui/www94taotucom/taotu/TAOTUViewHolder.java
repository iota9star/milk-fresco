package f.star.iota.milk.ui.www94taotucom.taotu;
...
public class TAOTUViewHolder extends BaseViewHolder<TAOTUBean> {
    ...
    public TAOTUViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<TAOTUBean> beans) {
        final TAOTUBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<TAOTUBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (TAOTUBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_94TAOTU,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
