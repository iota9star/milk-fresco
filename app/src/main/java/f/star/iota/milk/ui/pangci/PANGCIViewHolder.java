...
public class PANGCIViewHolder extends BaseViewHolder<PANGCIBean> {
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<PANGCIBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (PANGCIBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getPreview(), Menus.MENU_PANGCI,
