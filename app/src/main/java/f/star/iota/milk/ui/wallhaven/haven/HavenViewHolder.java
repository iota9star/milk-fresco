...
public class HavenViewHolder extends BaseViewHolder<HavenBean> {
    ...
    public HavenViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<HavenBean> beans) {
        final HavenBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<HavenBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (HavenBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_WALLHAVEN,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
