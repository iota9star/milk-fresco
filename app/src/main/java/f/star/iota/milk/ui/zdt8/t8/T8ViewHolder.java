...
public class T8ViewHolder extends BaseViewHolder<T8Bean> {
    @BindView(R.id.card_view)
    CardView mCardView;
    @BindView(R.id.simple_drawee_view_image)
    SimpleDraweeView mSimpleDraweeView;

    public T8ViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<T8Bean> beans) {
        final T8Bean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<T8Bean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (T8Bean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_ZDT8,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
