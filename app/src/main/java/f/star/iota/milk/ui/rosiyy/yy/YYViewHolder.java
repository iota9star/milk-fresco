...
public class YYViewHolder extends BaseViewHolder<YYBean> {
    ...
    @Override
    public void bindView(final List<YYBean> beans) {
        final YYBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<YYBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (YYBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_ROSIYY,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
