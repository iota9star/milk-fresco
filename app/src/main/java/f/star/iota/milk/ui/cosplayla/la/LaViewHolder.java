...
public class LaViewHolder extends BaseViewHolder<LaBean> {
    ...
    @Override
    public void bindView(final List<LaBean> beans) {
        final LaBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<LaBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (LaBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_COSPLAY_LA,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
