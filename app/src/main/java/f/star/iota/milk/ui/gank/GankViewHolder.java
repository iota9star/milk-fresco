...
public class GankViewHolder extends BaseViewHolder<GankBean.ResultsBean> {
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<GankBean.ResultsBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (GankBean.ResultsBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_GANK,
                    "描述：" + bean.getDesc() + "\n\n" +
                            "下载地址：" + bean.getUrl()));
        }
        return imgs;
