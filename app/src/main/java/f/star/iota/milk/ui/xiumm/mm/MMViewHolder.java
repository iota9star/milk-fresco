...
public class MMViewHolder extends BaseViewHolder<MMBean> {
    ...
    @Override
    public void bindView(final List<MMBean> beans) {
        final MMBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<MMBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (MMBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_XIUMM,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
