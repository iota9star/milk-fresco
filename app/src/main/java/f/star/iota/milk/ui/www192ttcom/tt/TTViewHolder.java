...
public class TTViewHolder extends BaseViewHolder<TTBean> {
    ...
    @Override
    public void bindView(final List<TTBean> beans) {
        final TTBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<TTBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (TTBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_192TT,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
