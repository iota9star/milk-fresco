...
public class MEIViewHolder extends BaseViewHolder<MEIBean> {
    ...
    @Override
    public void bindView(final List<MEIBean> beans) {
        final MEIBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<MEIBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (MEIBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_UMEI,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
