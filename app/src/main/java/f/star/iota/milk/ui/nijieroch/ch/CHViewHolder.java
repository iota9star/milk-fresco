package f.star.iota.milk.ui.nijieroch.ch;
...
public class CHViewHolder extends BaseViewHolder<CHBean> {
    ...
    @Override
    public void bindView(final List<CHBean> beans) {
        final CHBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<CHBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (CHBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_NIJIERO_CH,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
