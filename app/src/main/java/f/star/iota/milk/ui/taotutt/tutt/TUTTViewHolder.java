...
public class TUTTViewHolder extends BaseViewHolder<TUTTBean> {
    @BindView(R.id.card_view)
    CardView mCardView;
    @BindView(R.id.simple_drawee_view_image)
    SimpleDraweeView mSimpleDraweeView;

    public TUTTViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<TUTTBean> beans) {
        final TUTTBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<TUTTBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (TUTTBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_TAOTUTT,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
