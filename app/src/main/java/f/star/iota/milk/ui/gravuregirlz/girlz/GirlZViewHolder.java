...
public class GirlZViewHolder extends BaseViewHolder<GirlZBean> {
    @BindView(R.id.card_view)
    CardView mCardView;
    @BindView(R.id.simple_drawee_view_image)
    SimpleDraweeView mSimpleDraweeView;

    public GirlZViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<GirlZBean> beans) {
        final GirlZBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<GirlZBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (GirlZBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_GRAVUREGIRLZ,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
