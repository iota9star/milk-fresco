...
public class IngViewHolder extends BaseViewHolder<IngBean> {
    ...
    @Override
    public void bindView(final List<IngBean> beans) {
        final IngBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<IngBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (IngBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_BCY,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
