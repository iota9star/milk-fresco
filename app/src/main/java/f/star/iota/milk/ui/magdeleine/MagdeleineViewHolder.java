...
public class MagdeleineViewHolder extends BaseViewHolder<MagdeleineBean> {
    @BindView(R.id.simple_drawee_view_image)
    SimpleDraweeView mSimpleDraweeView;
    @BindView(R.id.card_view)
    CardView mCardView;

    public MagdeleineViewHolder(View itemView) {
        super(itemView);
    }

    @Override
    public void bindView(final List<MagdeleineBean> beans) {
        final MagdeleineBean bean = beans.get(getAdapterPosition());
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<MagdeleineBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (MagdeleineBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getPreview(), Menus.MENU_MAGDELEINE,
                    "下载地址：" + bean.getUrl()
            ));
        }
        return imgs;
