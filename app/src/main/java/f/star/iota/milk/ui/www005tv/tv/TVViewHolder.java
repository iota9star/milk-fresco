...
public class TVViewHolder extends BaseViewHolder<TVBean> {
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<TVBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (TVBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getUrl(), Menus.MENU_WWW_005_TV,
                    "下载地址：" + bean.getUrl()));
        }
        return imgs;
    }
