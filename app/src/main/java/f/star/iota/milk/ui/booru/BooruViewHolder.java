...
public class BooruViewHolder extends BaseViewHolder<BooruBean> {
    ...
    @Override
    protected List<PCBean> getProcessingCompletedBeans(List<BooruBean> beans) {
        List<PCBean> imgs = new ArrayList<>();
        for (BooruBean bean : beans) {
            imgs.add(new PCBean(bean.getUrl(), bean.getPreview(), Menus.MENU_YANDE + "_" + Menus.MENU_KONACHAN + "_" + Menus.MENU_LOLIBOORU,
                    "作者：" + bean.getAuthor() + "\n\n" +
                            "评级：" + bean.getRating() + "\n\n" +
                            "分辨率：" + bean.getSize() + "\n\n" +
                            "来源：" + bean.getSource() + "\n\n" +
